	...
	<main class="container max-w-screen-md px-5 pt-4 md:pt-8 pb-12 mx-auto flex flex-col">
		...
		<div class="flex flex-col space-y-6">
			<form class="flex flex-col space-y-2" on:submit|preventDefault={updateName}>
				...
				/>
				<button
					class="self-end text-white bg-green-500 border-0 py-2 px-8 focus:outline-none hover:bg-green-600 rounded text-lg"
					type="submit"
			...
			<form class="flex flex-col space-y-2" on:submit|preventDefault={updateEmail}>
				...
				<input
					...
					class="w-full bg-white rounded border border-gray-300 focus:border-green-500 focus:ring-2 focus:ring-green-200 text-base outline-none text-gray-700 py-1 px-3 leading-8 transition-colors duration-200 ease-in-out"
					type="email"
					placeholder="New Email Confirmation"
					bind:value={emailConf}
				/>
				{#if emailSuccess}
				...
				<button
					...
					class="disabled:opacity-50 self-end text-white bg-green-500 border-0 py-2 px-8 focus:outline-none hover:bg-green-600 rounded text-lg"
					type="submit"
					name="button">Save</button
				>
				<hr />
